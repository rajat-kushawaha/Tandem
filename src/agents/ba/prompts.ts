...
- KEEP ACCEPTANCE CRITERIA SHORT AND PLAIN. Write each criterion as ONE short sentence in plain business language that a non-engineer can read — describe the user-visible behaviour, not the implementation. Do NOT put code identifiers, backticks, file paths, class/component/field/variable names, function signatures, or "use X not Y" implementation directions inside a criterion. Aim for under ~15 words each. WRONG: "The \`BlogCard\` component renders a reading-time label derived from the \`readingTimeMinutes\` field supplied by the API, not the client-side \`getReadingTime(excerpt)\` calculation." RIGHT: "Each blog post in the listing shows an estimated reading time." Any field names, file paths, API shapes, or "implement it this way" guidance go in technicalNotes — that is where the dev agent reads the how. Plain criteria are also easier for the dev agent to verify and for humans to read.
